...
export function AtomCanvasGreen() {
  ...
  useEffect(() => {
    ...
    function drawRing(orbit: typeof orbits[0], alpha: number) {
      ...
      ctx!.strokeStyle = `rgba(196,168,79,${alpha})`
      ctx!.lineWidth = 0.7
      ctx!.stroke()
      ctx!.restore()
    }
    ...
    function drawTrail(trail: { x: number; y: number }[], maxSize: number) {
      for (let i = 0; i < trail.length; i++) {
        ...
        ctx!.fillStyle = `rgba(196,168,79,${alpha})`
        ctx!.fill()
      }
    }
    ...
    function drawElectron(pos: { x: number; y: number }, size: number, alpha: number) {
      ...
      grd.addColorStop(0, `rgba(196,168,79,${alpha * 0.5})`)
      grd.addColorStop(1, "rgba(196,168,79,0)")
      ctx!.beginPath()
    ...
    function drawNucleus(t: number) {
      ...
      ;[
        ...
        g.addColorStop(0, `rgba(45,107,71,${a})`)
        g.addColorStop(1, "rgba(45,107,71,0)")
        ctx!.beginPath()
      ...
      ctx!.strokeStyle = `rgba(45,107,71,${0.12 + pulse * 0.1})`
      ctx!.lineWidth = 1
      ctx!.stroke()

      // Green core
      const grad = ctx!.createRadialGradient(cx - 2, cy - 2, 1, cx, cy, 9)
      grad.addColorStop(0, "#6bc48e")
      grad.addColorStop(0.4, "#2d6b47")
      grad.addColorStop(1, "rgba(20,60,35,0.7)")
      ctx!.beginPath()
      ...
      nucleusParticles.forEach(p => {
        ...
        ctx!.fillStyle = `rgba(80,160,110,${0.18 + 0.28 * Math.sin(t * 1.1 + p.phase)})`
        ctx!.fill()
      })
    }
    ...
    function drawDust(t: number) {
      dust.forEach(d => {
        ...
        ctx!.fillStyle = `rgba(196,168,79,${a})`
        ctx!.fill()
      })
    }
    ...
    function frame(ts: number) {
      ...
      bg.addColorStop(0, "rgba(20,80,50,0.06)")
